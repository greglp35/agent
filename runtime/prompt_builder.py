...
def build_command_request(step: Dict[str, Any], command_spec: Dict[str, Any], *,
                          expert_specs: Iterable[Dict[str, Any]] = (),
...
"""
    if expert_block:
        system += "\nExpertises activées:\n" + expert_block
    user_payload={
        "command":step.get("command"),"mission":command_spec.get("mission"),"target":step.get("target"),
    ...
    contract={"required_outputs":command_spec.get("default_output",[]),
              "required_meta":["summary","epistemic","confidence"],
              "optional_meta":["tool_requests"],
              "epistemic_keys":["facts","inferences","assumptions","unknowns"]}
    return LLMRequest(purpose="command_execution",system=system,
                      user=json.dumps(user_payload,ensure_ascii=False,indent=2),
                      metadata={"command":step.get("command"),"depth":step.get("depth"),"target":step.get("target")},
...
"""
    return LLMRequest(purpose="council_expert_view",system=system,
        user=json.dumps({"target":target,"expert":expert,"context":context},ensure_ascii=False,indent=2),
        metadata={"expert_id":expert.get("id"),"target":target},
        response_contract={"required_meta":["summary","outputs","epistemic","confidence"]})
...
Ne fabrique pas de consensus. Une divergence non résolue doit rester visible.
"""
    return LLMRequest(purpose="council_arbitration",system=system,
        user=json.dumps({"target":target,"independent_views":views,"context":context},ensure_ascii=False,indent=2),
        metadata={"target":target},
        response_contract={"required_meta":["summary","disagreements","outputs","epistemic","confidence"]})
