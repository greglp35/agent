...
class SemanticRuntime:
    def __init__(self, llm: LLMAdapter, *, tool_adapters: Dict[str, Any] | None = None, council_parallel: bool = False, council_max_experts: int = 5):
        self.llm=llm; self.registries=load_registries(); self.tool_router=ToolRouter(tool_adapters)
    ...
    def _selected_expert_specs(self, compiled):
        routing=compiled.get("ir",{}).get("routing",{}); ids=[]
    ...
    def execute(self, compiled, *, state=None, provided_capabilities=None):
        capabilities=dict(provided_capabilities or {}); capabilities["llm.reason"]=True
        ...
        for raw_step in compiled.get("plan",{}).get("steps",[]):
            if not raw_step.get("command"):
                results.append({"phase":raw_step.get("phase"),"command":None,"status":"EXECUTED","content":{"routing":raw_step.get("details",{})}}); continue
            step=resolve_continuity(raw_step,state); command=step.get("command")
            if command=="council":
                try:
                    context={"previous_outputs":previous_outputs,"depth":step.get("depth"),"profile":step.get("profile"),"output_preferences":step.get("output_preferences",{})}
                    content=self.council.run(step.get("target"),compiled["ir"].get("routing",{}),context=context); status=content.pop("status","EXECUTED")
                except Exception as exc: content={"error":str(exc)}; status="BLOCKED"
                result={"phase":step.get("phase"),"command":command,"status":status,"content":content}; results.append(result)
            ...
            if not step.get("target") and command in {"continue","next","deeper","fix","redo","focus","final"}:
                results.append({"phase":step.get("phase"),"command":command,"status":"BLOCKED","content":{"error":"No active work state could resolve the continuity target."}}); continue
            try:
                ...
                request=build_command_request(step,command_spec,expert_specs=expert_specs,previous_outputs=previous_outputs,tool_context=tool_context,available_tools=self.tool_router.available())
                ...
                result={"phase":step.get("phase"),"command":command,"status":"PARTIALLY_EXECUTED" if blocked_tool else "EXECUTED","content":content,
                        "model":response.model,"usage":response.usage,"tool_context_used":sorted(tool_context.keys()),"tool_results":tool_results}
                results.append(result); previous_outputs.append({"command":command,"content":content})
            except Exception as exc: results.append({"phase":step.get("phase"),"command":command,"status":"BLOCKED","content":{"error":str(exc)}})
        ...
        return {"execution_status":overall,"host":getattr(self.llm,"name",type(self.llm).__name__),"capabilities":cap,"steps":results,"quality_gate":quality,
                "final_output":previous_outputs[-1] if previous_outputs else None,"budget":getattr(getattr(self.llm,"budget",None),"snapshot",lambda:None)()}
